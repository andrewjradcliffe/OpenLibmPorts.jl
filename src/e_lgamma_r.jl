...
function loggamma_r(x::Float64)
    ...
    #= purge off +-inf, NaN, +-0, tiny and negative arguments =#
    ix = hx & 0x7fffffff
    ix ≥ 0x7ff00000 && return typemax(x)
    ix | lx == 0x00000000 && return typemax(x)
    if ix < 0x3b900000 #= |x|<2**-70, return -log(|x|) =#
        hx < 0 && throw(DomainError(x, "`gamma(x)` must be non-negative"))
        return -log(x)
    end
    if hx < 0
        # ix ≥ 0x43300000 && return typemax(x) #= |x|>=2**52, must be -integer =#
        t = sinpi(x)
        iszero(t) && return typemax(x) #= -integer =#
        nadj = log(π / abs(t * x))
