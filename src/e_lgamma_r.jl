...
function loggamma_r(x::Float64)
    ...
    if hx < 0
        ...
        nadj = log(π / abs(t * x))
        t < 0 && throw(DomainError(x, "`gamma(x)` must be non-negative"))
        x = -x
    end
...
function loggamma_r2(x::Float64)
    u = reinterpret(UInt64, x)
    hx = (u >>> 32) % Int32
    lx = u % Int32
...
function loggamma_r3(x::Float64)
    u = reinterpret(UInt64, x)
    hx = (u >>> 32) % Int32
    lx = u % Int32
...
function loggamma_r4(x::Float64)
    u = reinterpret(UInt64, x)
    hx = (u >>> 32) % Int32
    lx = u % Int32
...
function loggamma_r5(x::Float64)
    u = reinterpret(UInt64, x)
    hx = (u >>> 32) % Int32
    lx = u % Int32
